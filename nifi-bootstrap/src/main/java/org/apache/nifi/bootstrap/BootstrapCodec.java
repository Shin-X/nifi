...
public class BootstrapCodec {
    ...
    public void communicate() throws IOException {
        ...
        if (splits.length < 0) {
            throw new IOException("从NiFi收到无效命令: " + line);
//            throw new IOException("Received invalid command from NiFi: " + line);
        ...
        try {
            ...
            throw new IOException("从NiFi收到无效命令: " + line + (ice.getMessage() == null ? "" : " - Details: " + ice.toString()));
        ...
        switch (cmd) {
            case "PORT": {
                ...
                try {
                    ...
                    throw new InvalidCommandException("无效的端口号;取值范围为1 ~ 65535之间的整数");
...
//                    throw new InvalidCommandException("Invalid Port number; should be integer between 1 and 65535");
            ...
            case "STARTED": {
                if (args.length != 1) {
                    throw new InvalidCommandException("已启动命令必须包含一个状态参数");
//                    throw new InvalidCommandException("STARTED command must contain a status argument");
                ...
                if (!"true".equals(args[0]) && !"false".equals(args[0])) {
                    throw new InvalidCommandException("启动命令的状态无效;应该是对的还是错的 '" + args[0] + "'");
                }
