        ...
        return $.Deferred(function (deferred) {
            ...
            if (isSaveRequired()) {
                ...
                nfDialog.showYesNoDialog({
                    headerText: /*'Processor Configuration'*/'处理器配置',
    ...
    return {
        ...
        init: function (options) {
            ...
            $('#processor-configuration').modal({
                ...
                headerText: /*'Configure Processor'*/'配置处理器',
                handler: {
                    close: function () {
                                        ...
                                        var cb = function(){
                                            ...
                                            if(nfCommon.getKeyValue(p,ACTIVE_THREAD_COUNT_KEY) != 0){
                                                nfDialog.showOkDialog({
                                                    ...
                                                    dialogContent: '请求被处理了，但是活动线程仍然存在。请稍后再试。',
                                                    headerText: /*'Unable to Terminate'*/'无法终止'
                                                });
                                            }
                                            else {
                                                //refresh the dialog
