        ...
        $.Deferred(function (deferred) {
            ...
            if (canceled === true && $('#nf-ok-dialog').not(':visible')) {
                nfDialog.showOkDialog({
                    ...
                    dialogContent: '禁用的请求已被取消。该请求的部分内容可能已经完成。请验证此服务和所有引用组件的状态。'
                });
            }
        });
    };
        ...
        $.Deferred(function (deferred) {
            ...
            if (canceled === true && $('#nf-ok-dialog').not(':visible')) {
                nfDialog.showOkDialog({
                    ...
                    dialogContent: '启用的请求已被取消。该请求的部分内容可能已经完成。请验证此服务和所有引用组件的状态。'
                });
            }
        });
    };
        ...
        return $.Deferred(function (deferred) {
            ...
            if (isSaveRequired()) {
                ...
                nfDialog.showYesNoDialog({
                    ...
                    dialogContent: '在进入控制器服务之前是否保存更改?',
                    noHandler: function () {
                        deferred.resolve();
                    },
                    yesHandler: function () {
    ...
    var nfControllerService = {
        ...
        init: function (nfControllerServicesRef, nfReportingTaskRef) {
            ...
            $('#controller-service-configuration').modal({
                headerText: /*'Configure Controller Service'*/'配置控制器服务',
            ...
            $('#disable-controller-service-dialog').modal({
                headerText: /*'Disable Controller Service'*/'禁用控制器服务',
                scrollableContentStyle: 'scrollable',
                handler: {
                    close: function () {
            ...
            $('#enable-controller-service-dialog').modal({
                headerText: /*'Enable Controller Service'*/'启用控制器服务',
                scrollableContentStyle: 'scrollable',
                handler: {
                    close: function () {
        ...
        promptToDeleteController: function (serviceTable, controllerServiceEntity) {
            // prompt for deletion
            nfDialog.showYesNoDialog({
                headerText: /*'Delete Controller Service'*/'删除控制器服务',
