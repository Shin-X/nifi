...
(function (root, factory) {
    ...
    var initAddTenantToPolicyDialog = function () {
        ...
        $('#search-users-dialog').modal({
            ...
            headerText: /*'Add Users/Groups'*/'添加用户/组',
            buttons: [{
                buttonText: /*'Add'*/'添加',
                color: {
                    base: '#728E9B',
    ...
    var initPolicyTable = function () {
        $('#override-policy-dialog').modal({
            headerText: /*'Override Policy'*/'重载策略',
            buttons: [{
                buttonText: /*'Override'*/'重载',
                color: {
                    base: '#728E9B',
                    hover: '#004849',
                    text: '#ffffff'
    ...
    var promptToRemoveUserFromPolicy = function (item) {
        nfDialog.showYesNoDialog({
            headerText: /*'Update Policy'*/'更新策略',
            // dialogContent: 'Remove \'' + nfCommon.escapeHtml(item.component.identity) + '\' from this policy?',
            dialogContent: '删除 \'' + nfCommon.escapeHtml(item.component.identity) + '\' 从这个策略中?',
            yesHandler: function () {
    ...
    var promptToDeletePolicy = function () {
        nfDialog.showYesNoDialog({
            ...
            dialogContent: '通过删除此策略，此组件的权限将恢复为继承的策略(如果适用)。',
            yesText: 'Delete',
            noText: 'Cancel',
            yesHandler: function () {
                deletePolicy();
