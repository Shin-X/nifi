    ...
    var updateResource = function (uri, entity) {
        ...
        return $.ajax({
            ...
            nfDialog.showOkDialog({
                headerText: /*'Update Resource'*/'更新资源',
    ...
    var nfActions = {
        ...
        openUri: function (selection) {
            if (selection.size() === 1 && nfCanvasUtils.isRemoteProcessGroup(selection)) {
                ...
                if (!nfCommon.isBlank(uri)) {
                    ...
                    nfDialog.showOkDialog({
                        headerText: /*'Remote Process Group'*/'远程进程组',
                        dialogContent: '没有定义目标URI。'
        ...
        'delete': function (selection) {
            if (nfCommon.isUndefined(selection) || selection.empty()) {
                nfDialog.showOkDialog({
                    headerText: /*'Delete Components'*/'删除组件',
                    dialogContent: '没有选择合格的组件。请选择要删除的组件。'
        ...
        emptyQueue: function (selection) {
            ...
            nfDialog.showYesNoDialog({
                headerText: /*'Empty Queue'*/'清空队列',
                dialogContent: '您确定要清空此队列吗?在请求时等待的所有流程文件将被删除。',
        ...
        emptyAllQueues: function (selection) {
            // prompt the user before emptying the queue
            nfDialog.showYesNoDialog({
                headerText: /*'Empty All Queues'*/'清空所有队列',
                dialogContent: '您确定要清空此进程组中的所有队列吗?请求时等待的所有连接中的所有流文件将被删除。',
        ...
        forceSaveFlowVersion: function (selection) {
            nfDialog.showYesNoDialog({
                headerText: /*'Commit'*/'提交',
                dialogContent: '提交将忽略可用的升级，并将本地更改提交为下一个版本。您确定要继续吗?',
        ...
        downloadFlow: function (selection) {
            ...
            if (processGroupId !== null) {
                nfCommon.getAccessToken(config.urls.downloadToken).done(function (downloadToken) {
                    ...
                    nfDialog.showOkDialog({
                        headerText: /*'Download Flow Definition'*/'下载流定义',
        ...
        alignVertical: function (selection) {
            ...
            if (nfCanvasUtils.canModify(selection) === false) {
                nfDialog.showOkDialog({
                    headerText: /*'Component Position'*/'组件的位置',
                    dialogContent: '必须经过授权才能修改所选的每个组件。'
        ...
        template: function () {
            ...
            if (selection.empty()) {
                nfDialog.showOkDialog({
                    headerText: /*'Create Template'*/'创建模板',
                ...
                handler: {
                    click: function () {
                        ...
                        if (nfCommon.isBlank(templateName)) {
                            nfDialog.showOkDialog({
                                headerText: /*'Configuration Error'*/'配置错误',
                                dialogContent: "必须指定模板的名称。"
