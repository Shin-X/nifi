...
@Tags({"excel", "csv", "poi"})
@CapabilityDescription("使用Microsoft Excel文档并将每个工作表转换为csv。来自传入Excel文档的每个工作表将生成一个新的流文件，" +
        "该流文件将由该处理器输出。每个输出流文件的内容将被格式化为csv文件，其中excel表中的每一行将在csv文件中以换行符的形式输出。" +
        "这个处理器目前只能处理。xlsx (XSSF 2007 OOXML文件格式)Excel文档，而不能处理更老的。xls (HSSF '97(-2007)文件格式)文档。" +
        "此处理程序还需要格式化良好的CSV内容，并且不会转义包含无效内容(如换行符或额外逗号)的单元格。")
