...
@CapabilityDescription("使用提供的记录读取器从传入的流文件读取记录，并将这些记录写入Parquet文件。必须在处理器属性中提供拼花文件的模式。该处理器将首先写入一个临时的点文件，在成功地将每个记录写入点文件后，它将把点文件重命名为它的最终名称。如果不能重命名点文件，重命名操作将尝试最多10次，如果仍然不成功，点文件将被删除，流文件将被路由到失败。如果在从输入读取记录或将记录写入输出时发生任何错误，则将删除整个点文件，并将将流文件路由到失败或重试，具体取决于错误。")
