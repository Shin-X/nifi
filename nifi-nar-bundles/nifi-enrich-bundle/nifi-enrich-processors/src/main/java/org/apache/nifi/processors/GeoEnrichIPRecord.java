...
@CapabilityDescription("查找IP地址的地理位置信息，并将地理信息添加到FlowFile属性中。geo数据以MaxMind数据库的形式提供。这个版本使用了NiFi Record API，允许大规模地丰富面向记录的数据集。通过为字段配置提供记录路径，MaxMind数据库提供的每个字段都可以被定向到用户选择的字段。")
