...
@CapabilityDescription("查找IP地址的ISP信息，并将该信息添加到FlowFile属性中。ISP数据以MaxMind ISP数据库的形式提供(注意这与某些geo浓缩工具使用的GeoLite数据库不同)。包含要查找的IP地址的属性由“IP地址属性”属性提供。如果提供的属性名是'X'，那么通过富集添加的属性将采用X.isp.&lt;fieldName&gt;")
@WritesAttributes({
