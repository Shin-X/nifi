...
@CapabilityDescription("根据给定的模式验证传入流文件的记录。所有遵循该模式的记录都被路由到\"valid\"关系，而不遵循该模式的记录则被路由到\"invalid\"关系。因此，如果一些记录根据模式是有效的，而另一些则不是，那么一个传入的流文件可能被拆分为两个单独的流文件。任何被路由到\"invalid\"关系的流文件都将发出一个ROUTE起源事件，并填充Details字段来解释为什么记录是无效的。此外，为了进一步解释为什么记录是无效的，可以为\\\"org.apache.nifi.processors.standard启用调试级日志记录。ValidateRecord \"记录器。")
@WritesAttributes({
