...
@CapabilityDescription(
        "对一个流文件的内容计算一个或多个xquery。这些x查询的结果被分配给流文件属性，或者被写入流文件本身的内容，这取决于处理器的配置。" +
                "通过添加用户定义的属性来输入xquery;属性的名称映射到结果将被放置的属性名(如果目标是'flowfile-attribute';否则，将忽略属性名)。" +
                "属性的值必须是有效的XQuery。如果XQuery返回多个结果，则会为每个结果创建新的属性或流文件" +
                "(分别用于'flowfile-attribute'或'flowfile-content'的目的地)(属性将有一个'。n' one-up number附加到指定的属性名)。" +
                "如果提供的任何XQuery返回一个结果，那么该流文件将被路由到'matched'。如果没有提供的XQuery返回结果，那么该流文件将被路由到“unmatched”。" +
                "如果目标是' FlowFile -attribute'，并且XQueries不匹配任何内容，则不会将任何属性应用到该FlowFile。")
//@CapabilityDescription(
