...
@CapabilityDescription(
        "根据流文件的内容计算一个或多个正则表达式。这些正则表达式的结果被指定给FlowFile属性。正则表达式是通过添加用户定义的属性来输入的；" +
                "属性的名称映射到将结果放入其中的属性名称。第一个捕获组（如果有）将被放入该属性中名字。但是所有捕获组（包括匹配的字符串序列本身）也将在该属性名称处提供索引值，" +
                "但可选项且不匹配的捕获组除外（例如，给定属性名称“regex”）以及“abc（def）”表达式？（g） 如果“def”匹配，我们将添加一个值为“def”的属性“regex.1”。" +
                "如果“def”不匹配，则不会添加名为“regex.1”的属性，但会添加名为“regex.2”且值为“g”的属性不管怎样属性的值必须是包含一个或多个捕获组的有效正则表达式。" +
                "如果正则表达式匹配多次，则仅使用第一个匹配，除非启用重复捕获组的属性设置为true。如果提供的任何正则表达式匹配，则流文件将路由到“matched”。" +
                "如果没有提供匹配的正则表达式，则流文件将路由到“unmatched”，并且不会对流文件应用任何属性。")
//@CapabilityDescription(
