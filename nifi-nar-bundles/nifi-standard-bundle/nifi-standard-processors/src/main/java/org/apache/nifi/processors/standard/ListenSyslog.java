...
@CapabilityDescription("监听通过TCP或UDP发送到给定端口的Syslog消息。根据RFC5424和RFC3164格式消息的正则表达式检查传入消息。每条消息的格式为:(<PRIORITY>)(VERSION )(TIMESTAMP) (HOSTNAME) (BODY)，其中VERSION是可选的。时间戳可以是RFC5424时间戳，格式为“yyyy-MM-dd'T'HH:mm:ss”。深圳\"、\"yyyy-MM-dd 'HH: mm: ss。S+hh:mm\"，也可以是RFC3164时间戳，格式为“MMM d hh:mm:ss\"。如果传入的消息与这些模式之一相匹配，则消息将被解析，单个片段将被放置在FlowFile属性中，原始消息将被放置在FlowFile的内容中。如果传入消息不匹配这些模式之一，则不会对其进行解析和syslog。对于流文件内容中的原始消息，valid属性将被设置为false。在成功关系上传递有效消息，在无效关系上传递无效消息。")
