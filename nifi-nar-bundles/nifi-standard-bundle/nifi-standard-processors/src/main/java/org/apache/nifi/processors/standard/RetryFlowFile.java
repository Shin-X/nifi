...
@Tags({"Retry", "FlowFile"})
@CapabilityDescription("传递到此处理器的流程文件有一个“重试属性”值，根据配置的“最大重试次数”值进行检查。如果当前属性值低于配置的最大值，则将流文件传递给重试关系。在这种情况下，流程文件可能会受到惩罚，也可能不会。如果该流文件的属性值超过了配置的最大值，那么该流文件将被传递给'retries_exceeded'关系。警告:如果传入的流文件在配置的“重试属性”属性中具有非数字值，它将被重置为“1”。您可以选择让流文件失败，而不是执行重置。您可以为任何希望添加到传输到'retries_exceeded'的流文件中的属性定义额外的动态属性。这些属性支持属性表达式语言。")
