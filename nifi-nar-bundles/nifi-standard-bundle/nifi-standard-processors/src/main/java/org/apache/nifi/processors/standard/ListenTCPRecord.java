...
@CapabilityDescription("监听传入的TCP连接，并使用配置的记录读取器从每个连接读取数据，并使用配置的记录写入器将记录写入流文件。所选记录阅读器的类型将决定期望客户机如何发送数据。例如,当使用一个神交读者阅读日志、客户端可以保持开放的连接和连续流数据,但是当使用一个JSON读者,客户机不能发送JSON文档,然后发送另一个数组的数组相同的连接,我们将是在一个糟糕的状态。记录将以阻塞模式从连接中读取，并将根据处理器中指定的读取超时时间超时。如果读取超时，或者在读取时遇到任何其他错误，连接将被关闭，读取到该点的任何记录将根据配置的读错误策略(丢弃或传输)进行处理。在客户端保持连接打开的情况下，处理器的并发任务应该调整到与允许的最大TCP连接数相匹配，这样每个连接都有一个任务来处理。")
@WritesAttributes({
