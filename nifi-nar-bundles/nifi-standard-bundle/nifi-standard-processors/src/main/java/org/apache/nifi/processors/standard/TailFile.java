...
@CapabilityDescription("\"Tails\"一个文件或文件列表，在将数据写入文件时从文件中获取数据。文件应该是文本的。数据仅在遇到新行(回车或换行字符或组合)时才被接收。如果文件定期尾巴\\“翻滚\\”,就像一般的日志文件,一个可选的滚动文件名模式可用于检索数据从文件翻滚,即使发生翻转NiFi不是运行时(提供数据仍然存在在重启NiFi)。通常建议将运行计划设置为几秒，而不是使用默认值0秒运行，因为如果非常主动地调度，这个处理器将消耗大量资源。此时，这个处理器不支持摄取在“rolled over”时被压缩的文件。")
