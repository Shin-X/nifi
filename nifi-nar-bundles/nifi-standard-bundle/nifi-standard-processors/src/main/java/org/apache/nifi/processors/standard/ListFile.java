...
@CapabilityDescription("从本地文件系统检索文件列表。对于列出的每个文件，创建一个表示该文件的流文件，以便可以与FetchFile一起获取该文件。这个处理器被设计成只在集群中的主节点上运行。如果主节点发生变化，新的主节点将从上一个节点停止的地方开始，而不会复制所有数据。与GetFile不同，这个处理器不会从本地文件系统中删除任何数据。")
@WritesAttributes({
