...
@CapabilityDescription("为每个传入的流文件缓存一个值，从流文件属性计算，并确定缓存的值是否已经被看到。如果是，将流文件路由到带有名为original属性的“duplicate”。" +
        "标识符'指定了原始流文件的\"description\"，它在<FlowFile Description> 属性中。如果该流文件不是重复的，处理器将该流文件路由到“non-duplicate”。")
