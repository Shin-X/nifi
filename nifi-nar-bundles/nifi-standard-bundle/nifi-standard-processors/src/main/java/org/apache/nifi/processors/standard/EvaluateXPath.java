...
@CapabilityDescription("根据一个流文件的内容计算一个或多个xpath。这些xpath的结果被分配给流文件属性，或者被写入流文件本身的内容，" +
        "这取决于处理器的配置。通过添加用户定义的属性来输入xpath;属性的名称映射到将放置结果的属性名(如果目标是flowfile-attribute;否则，将忽略属性名)。" +
        "属性的值必须是有效的XPath表达式。如果XPath计算为多个节点，并且返回类型设置为'nodeset'(直接或通过'auto-detect'，目的地为' FlowFile -content')，" +
        "则FlowFile将不被修改，并将被路由到失败。如果XPath没有计算到节点，那么流文件将被路由到“unmatched”，而不会修改其内容。" +
        "如果Destination是FlowFile -attribute并且表达式不匹配任何内容，属性将以空字符串作为值创建，并且FlowFile将始终被路由到'matched'")
