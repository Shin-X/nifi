...
@Tags({"Ship", "sql", "database", "rdbms", "insert", "update", "delete", "relational", "flat"})
@CapabilityDescription("Converts a Ship-formatted FlowFile into an UPDATE, INSERT, or DELETE SQL statement. The incoming FlowFile is expected to be "
        + "\"flat\" Ship message, meaning that it consists of a single Ship element and each field maps to a simple type. If a field maps to "
        + "a Ship object, that Ship object will be interpreted as Text. If the input is an array of Ship elements, each element in the array is "
...
@WritesAttributes({
        ...
        @WritesAttribute(attribute = "fragment.identifier", description = "All FlowFiles routed to the 'sql' relationship for the same incoming FlowFile (multiple will be output for the same incoming "
                + "FlowFile if the incoming FlowFile is a Ship Array) will have the same value for the fragment.identifier attribute. This can then be used to correlate the results."),
...
public class ConvertShipToSQL extends AbstractProcessor {
    ...
    static final AllowableValue IGNORE_UNMATCHED_FIELD = new AllowableValue("Ignore Unmatched Fields", "Ignore Unmatched Fields",
            "Any field in the Ship document that cannot be mapped to a column in the database is ignored");
    static final AllowableValue FAIL_UNMATCHED_FIELD = new AllowableValue("Fail", "Fail",
            "If the Ship document has any field that cannot be mapped to a column in the database, the FlowFile will be routed to the failure relationship");
    static final AllowableValue IGNORE_UNMATCHED_COLUMN = new AllowableValue("Ignore Unmatched Columns",
            "Ignore Unmatched Columns",
            "Any column in the database that does not have a field in the Ship document will be assumed to not be required.  No notification will be logged");
    static final AllowableValue WARNING_UNMATCHED_COLUMN = new AllowableValue("Warn on Unmatched Columns",
            ...
            "Any column in the database that does not have a field in the Ship document will be assumed to not be required.  A warning will be logged");
    static final AllowableValue FAIL_UNMATCHED_COLUMN = new AllowableValue("Fail on Unmatched Columns",
            "Fail on Unmatched Columns",
            "A flow will fail if any column in the database that does not have a field in the Ship document.  An error will be logged");
    public static final PropertyDescriptor ZIPDIRECTORY = new PropertyDescriptor.Builder()
            .name("ZIPDIRECTORY")
            ...
            .build();
    public static final PropertyDescriptor OUTDIRECTORY = new PropertyDescriptor.Builder()
            .name("OUTDIRECTORY")
            .description("The input directory from which to pull files")
            .required(true)
            //.addValidator(StandardValidators.createDirectoryExistsValidator(true, false))
            ...
            .expressionLanguageSupported(ExpressionLanguageScope.VARIABLE_REGISTRY)
            .build();
    public static final PropertyDescriptor SHIPDIRECTORY = new PropertyDescriptor.Builder()
            .name("SHIPDIRECTORY")
            .description("The input directory from which to pull files")
            .required(true)
            // .addValidator(StandardValidators.createDirectoryExistsValidator(true, false))
    ...
    static final PropertyDescriptor CONNECTION_POOL = new PropertyDescriptor.Builder()
            ...
            .description("Specifies the JDBC Connection Pool to use in order to convert the Ship message to a SQL statement. "
    ...
    static final PropertyDescriptor TRANSLATE_FIELD_NAMES = new PropertyDescriptor.Builder()
            ...
            .description("If true, the Processor will attempt to translate Ship field names into the appropriate column names for the table specified. "
                    + "If false, the Ship field names must match the column names exactly, or the column will not be updated")
    ...
    static final PropertyDescriptor UNMATCHED_FIELD_BEHAVIOR = new PropertyDescriptor.Builder()
            ...
            .description("If an incoming Ship element has a field that does not map to any of the database table's columns, this property specifies how to handle the situation")
    ...
    static final PropertyDescriptor UNMATCHED_COLUMN_BEHAVIOR = new PropertyDescriptor.Builder()
            ...
            .description("If an incoming Ship element does not have a field mapping for all of the database table's columns, this property specifies how to handle the situation")
    ...
    static final Relationship REL_ORIGINAL = new Relationship.Builder()
            .name("original")
            .description("When a FlowFile is converted to SQL, the original Ship FlowFile is routed to this relationship")
            .build();
    ...
    static final Relationship REL_FAILURE = new Relationship.Builder()
            .name("failure")
            .description("A FlowFile is routed to this relationship if it cannot be converted into a SQL statement. Common causes include invalid Ship "
                    + "content or the Ship content missing a required field (if using an INSERT statement type).")
            .build();
    ...
    protected List<PropertyDescriptor> getSupportedPropertyDescriptors() {
        ...
        properties.add(ZIPDIRECTORY);
        properties.add(OUTDIRECTORY);
        properties.add(SHIPDIRECTORY);
        return properties;
    }
    ...
    @Override
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException, IOException {
        ...
        // Parse the Ship document
        final ObjectMapper mapper = new ObjectMapper();
        final AtomicReference<JsonNode> rootNodeRef = new AtomicReference<>(null);
        //<editor-fold desc="shipTosql">
        final String zipdir = context.getProperty(ZIPDIRECTORY).evaluateAttributeExpressions().getValue();
        final String outdir = context.getProperty(OUTDIRECTORY).evaluateAttributeExpressions().getValue();
        // String outdir = "D:\\data\\nifi\\";
        // final String shipdir = context.getProperty(OUTDIRECTORY).evaluateAttributeExpressions().getValue();
        String shipdir = "D:\\data\\nifi\\采石场区\\日照市_采石场区.shp";
        String encode = "GBK";
        File zipFile = new File(zipdir);
        File shapefile = new File(shipdir);
        unZipFiles(zipFile, outdir);
...
Geometry geometry = (Geometry) property.getValue();
                            ...
                            // json 消息
                            json += "{\"srid\"" + ":" + "\"" + srid + "\"," + "\"geom\"" + ":" + "\"" + property.getValue() + "\"}";
                            // 将 json 转成 JsonNode 对象
                            break;
                        default:
                            // dataLine += property.getValue();
                            ///mapShip.put("all",property.getValue());
                            break;
                    }
                }
                json += ",";
            ...
            BufferedWriter printer = new BufferedWriter(new FileWriter(file), 5120);
            printer.write(json);
            printer.flush();
            printer.close();
        }
        ...
        try {
            ...
            getLogger().error("Failed to parse {} as Ship due to {}; routing to failure", new Object[]{flowFile, pe.toString()}, pe);
            session.transfer(flowFile, REL_FAILURE);
            return;
        }
        ...
        for (int i = 0; i < arrayNode.size(); i++) {
            ...
            try {
                ...
                if (INSERT_TYPE.equals(statementType)) {
                    ...
                    BufferedWriter printer = new BufferedWriter(new FileWriter(file), 5120);
                    BufferedWriter printer2 = new BufferedWriter(new FileWriter(file2), 5120);
                    ...
                    printer.flush();
                    printer.close();
                    printer2.write(jsonNode.toString());
                    printer2.flush();
                    printer2.close();
    ...
    // The node may or may not be a Ship Array. If it isn't, we will create an
    ...
    private String generateInsert(final JsonNode rootNode, final Map<String, String> attributes, final String tableName,
        ...
        for (final String requiredColName : schema.getRequiredColumnNames()) {
            ...
            if (!normalizedFieldNames.contains(normalizedColName)) {
                String missingColMessage = "Ship does not have a value for the Required column '" + requiredColName + "'";
...
// iterate over all of the elements in the Ship, building the SQL statement by adding the column names, as well as
...
// columns that we are inserting into
        ...
        while (fieldNames.hasNext()) {
            ...
            if (desc == null && !ignoreUnmappedFields) {
                throw new ProcessException("Cannot map Ship field '" + fieldName + "' to any column in the database");
            }
        ...
        if (fieldCount == 0) {
            throw new ProcessException("None of the fields in the Ship map to the columns defined by the " + tableName + " table");
        }
...
// Create a Set of all normalized Update Key names, and ensure that there is a field in the Ship
// for each of the Update Key fields.
        ...
        for (final String uk : updateKeyNames) {
            ...
            if (!normalizedFieldNames.contains(normalizedUK)) {
                String missingColMessage = "Ship does not have a value for the " + (updateKeys == null ? "Primary" : "Update") + "Key column '" + uk + "'";
                if (failUnmappedColumns) {
                    getLogger().error(missingColMessage);
                    throw new ProcessException(missingColMessage);
                } else if (warningUnmappedColumns) {
    ...
    private static class TableSchema {
        ...
        private TableSchema(final List<ColumnDescription> columnDescriptions, final boolean translateColumnNames,
            ...
            for (final ColumnDescription desc : columnDescriptions) {
                columns.put(ConvertShipToSQL.normalizeColumnName(desc.columnName, translateColumnNames), desc);
                if (desc.isRequired()) {
