...
@CapabilityDescription("请注意，此处理器已弃用，可能在不久的将来被移除。使用InvokeHTTP代替。从HTTP或HTTPS URL获取数据，并将数据写入流文件的内容。一旦获取了内容，就会记住ETag和最后修改的日期(如果web服务器支持这些概念的话)。这允许处理器仅在远程数据发生更改或状态被清除时才获取新数据。也就是说，一旦从给定的URL获取了内容，就不会再次获取它，直到远程服务器上的内容发生变化。注意，由于状态管理的限制，存储的\"last modified\"和etag字段永远不会过期。如果GetHttp中的URL使用了不受限制的表达式语言，则有可能发生内存外错误。")
@DynamicProperties({
