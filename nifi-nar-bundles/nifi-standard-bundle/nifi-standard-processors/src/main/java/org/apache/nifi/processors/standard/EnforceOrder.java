...
@TriggerSerially
@CapabilityDescription("强制在单个节点内属于同一数据组的流文件的预期顺序。尽管可以在连接上使用priorityattributepriority，" +
        "以确保经过该连接的流文件按照优先级顺序排列(取决于错误处理、分支和其他流设计)，但流文件也可能无序排列。强制执行命令可用于强制执行这些流程文件的原始命令。" +
        "[重要]为了使强制执行生效，应该在每个下游关系中使用firstinfirstoutpriority，直到流文件的顺序通过诸如合并内容或存储到最终目的地的操作得到物理上的固定。")
