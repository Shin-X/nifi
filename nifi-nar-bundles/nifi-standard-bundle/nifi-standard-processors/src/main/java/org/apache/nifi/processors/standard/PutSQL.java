...
@CapabilityDescription("执行SQL UPDATE或INSERT命令。传入流文件的内容应该是要执行的SQL命令。SQL命令可能会使用?为了逃避参数。在这种情况下，要使用的参数必须作为命名约定为sql.args.N的FlowFile属性存在。类型和sql.args.N。取值，其中N为正整数。sql.args.N。type应该是一个表示JDBC类型的数字。流文件的内容预计将采用UTF-8格式。")
@ReadsAttributes({
...
//                    + "The Connection Pool is necessary in order to determine the appropriate database column types.")
    ...
    static final PropertyDescriptor SQL_STATEMENT = new PropertyDescriptor.Builder()
            ...
            .displayName("SQL语句")
            .description("要执行的SQL语句。语句可以是空的、常量值，或者使用表达式语言从属性构建。如果指定了此属性，则无论传入流文件的内容如何，都将使用该属性。如果此属性为空，" +
                    "则传入流文件的内容将包含一个有效的SQL语句，该语句将由处理器向数据库发出。")
...
//                    + "to contain a valid SQL statement, to be issued by the processor to the database.")
            /*
            要执行的SQL语句。语句可以为空、常量值或根据属性生成
            +“使用表达语言。如果指定了此属性，则无论
            +“传入流文件。如果此属性为空，则需要传入流文件的内容“
            +包含由处理器向数据库发出的有效SQL语句。
             **/
            .required(false)
    ...
    static final PropertyDescriptor AUTO_COMMIT = new PropertyDescriptor.Builder()
            ...
            .description("在使用的数据库连接上设置的自动提交模式。如果设置为false，操作将显式提交或回滚(分别基于成功或失败)，如果设置为true，驱动程序/数据库将处理提交/回滚。")
...
//                    + "(based on success or failure respectively), if set to true the driver/database handles the commit/rollback.")
    ...
    static final PropertyDescriptor SUPPORT_TRANSACTIONS = new PropertyDescriptor.Builder()
            .name("支持的事务")
            .description("如果为真，当这个处理器使用一个流文件时，处理器将首先检查这个片段。标识符和片段。计数该流文件的属性。如果片段。如果计数值大于1，" +
                    "处理器将不会处理任何带有该片段的流文件。标识符，直到所有可用;此时，它将处理带有该片段的所有流程文件。标识符作为一个单独的事务，" +
                    "按照流程文件片段指定的顺序。索引属性。这提供了这些SQL语句的原子性。一旦该事务的任何语句在执行时抛出异常，该事务将被回滚。当事务回滚发生时，" +
                    "这些流文件都不会被路由到“success”。如果<回滚失败>设置为true时，这些流文件将保持在输入关系中。当<回滚失败>设置为false时，" +
                    "如果这些流程文件中的任何一个将被路由到'retry'，那么所有这些流程文件将被路由到'retry'。否则，它们将被路由到“失败”。如果该值为false，" +
                    "这些属性将被忽略，更新将彼此独立地进行。")
...
//                    + "they will be routed to 'failure'. If this value is false, these attributes will be ignored and the updates will occur independent of one another.")
    ...
    static final PropertyDescriptor TRANSACTION_TIMEOUT = new PropertyDescriptor.Builder()
            .name("事务超时")
            .description("如果<支持的事务>属性设置为true时，指定为某个特定片段等待所有流文件的时间。在将所有带有该标识符的流文件传输到“失败”关系之前，要到达的标识符属性")
...
//                    + "to arrive before just transferring all of the FlowFiles with that identifier to the 'failure' relationship")
    ...
    static final PropertyDescriptor BATCH_SIZE = new PropertyDescriptor.Builder()
            .name("批量大小")
            .description("在单个事务中放置到数据库中的首选流程文件数量")
//            .name("Batch Size")
//            .description("The preferred number of FlowFiles to put to the database in a single transaction")
    ...
    static final PropertyDescriptor OBTAIN_GENERATED_KEYS = new PropertyDescriptor.Builder()
            .name("获取生成密钥")
            .description("如果为true，则数据库自动生成的任何键都将添加到使用sql.generate生成它的流文件中。关键属性。这可能会导致性能稍慢，并且不是所有数据库都支持。")
...
//                    + "This may result in slightly slower performance and is not supported by all databases.")
    ...
    static final Relationship REL_SUCCESS = new Relationship.Builder()
            .name("成功")
            .description("成功更新数据库后，将一个流文件路由到此关系")
            .build();
    static final Relationship REL_RETRY = new Relationship.Builder()
            .name("重试")
            .description("如果数据库无法更新，但再次尝试操作可能会成功，那么将一个流文件路由到这个关系")
            .build();
    static final Relationship REL_FAILURE = new Relationship.Builder()
            .name("失败")
            .description("如果数据库无法更新，并且重试操作也会失败，例如无效查询或完整性约束违反，则将流文件路由到此关系")
            .build();
//    static final Relationship REL_SUCCESS = new Relationship.Builder()
//            .name("success")
...
//            .build();
     ...
     *
     * @param context:
     * @return java.util.Collection<org.apache.nifi.components.ValidationResult>
     * @author liuxin
    ...
    final PutGroup.GroupFlowFiles<FunctionContext, Connection, StatementFlowFileEnclosure> groupFlowFiles = (context, session, fc, conn, flowFiles, result) -> {
        ...
        //有三种模式：
        // 1. 支持批处理：一个存储模块在批处理操作中有多个正在执行的流文件
        // 2. 获取密钥：一个存储模块有多个流文件，每个流文件分别执行
        // 3. 碎片化事务：每个存储模块一个流文件？
        if (fc.obtainKeys) {
