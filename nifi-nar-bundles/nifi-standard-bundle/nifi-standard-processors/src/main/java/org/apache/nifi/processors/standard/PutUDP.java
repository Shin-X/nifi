 ...
 *
 */
@CapabilityDescription("puudp处理器接收一个流文件，并将流文件内容打包成一个UDP数据报包，然后传输到配置好的UDP服务器。用户必须确保提供给这个处理器的FlowFile内容不大于底层UDP传输的最大大小。最大传输大小将根据平台设置而变化，但通常略低于64KB。如果流文件的内容大于最大传输大小，则会被标记为失败。")
