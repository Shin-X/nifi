 ...
 *
 */
@CapabilityDescription("PutTCP处理器接收一个流文件，并通过TCP连接将流文件内容传输到已配置的TCP服务器。缺省情况下，这些流文件通过同一个TCP连接(如果配置了多个输入线程，则使用TCP连接池)传输。为了帮助TCP服务器确定消息边界，可以配置一个可选的\"传出消息分隔符\"字符串，该字符串在通过TCP连接传输时附加到每个flowfile内容的末尾。可以指定一个可选的\"Connection Per FlowFile\"参数来改变行为，以便每个FlowFile内容通过一个单独的TCP连接传输，当FlowFile被接收时打开，并在FlowFile发送后关闭。此选项应仅用于消息量较小的场景，否则平台“+”可能耗尽TCP套接字。")
