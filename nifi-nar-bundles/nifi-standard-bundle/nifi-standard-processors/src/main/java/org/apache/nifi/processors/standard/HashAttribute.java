...
@CapabilityDescription("将几个流文件属性的键/值对散列在一起，并将散列添加为一个新属性。添加可选的属性,这样的名称属性的名称flowfile属性考虑和属性的值是一个正则表达式,如果匹配的属性值,会导致属性作为散列的一部分。如果正则表达式包含捕获组，则只使用捕获组的值。\" + \"表示接受各种属性并为每个属性生成加密哈希的处理器，请参见\"CryptographicHashAttribute\"。")
