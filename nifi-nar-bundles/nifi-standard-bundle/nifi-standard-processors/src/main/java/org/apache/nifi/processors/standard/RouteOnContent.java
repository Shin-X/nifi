...
@CapabilityDescription("应用正则表达式到流文件的内容，并将该流文件的副本路由到正则表达式匹配的每个目的地。正则表达式作为用户定义的属性添加，其中属性的名称是关系的名称，值是用于匹配FlowFile内容的正则表达式。用户定义的属性确实支持属性表达式语言，但是结果被解释为文字值，而不是正则表达式")
