...
@CapabilityDescription("执行提供的SQL select查询。查询结果将被转换为记录写入器指定的格式。使用流是为了支持任意大的结果集。" +
        "可以使用标准调度方法将该处理器调度为在计时器或cron表达式上运行，也可以由传入的流文件触发。如果它是由一个传入的流文件触发的，那么当评估选择查询时，" +
        "该流文件的属性将是可用的，并且查询可以使用?为了逃避参数。在这种情况下，要使用的参数必须作为命名约定为sql.args.N的FlowFile属性存在。" +
        "类型和sql.args.N。取值，其中N为正整数。sql.args.N。type应该是一个表示JDBC类型的数字。流文件的内容预计将采用UTF-8格式。“executesql.row FlowFile属性。指示被选中的行数。")
...
public class ExecuteSQLRecord extends AbstractExecuteSQL {
    ...
    public static final PropertyDescriptor RECORD_WRITER_FACTORY = new PropertyDescriptor.Builder()
            ...
            .displayName("记录的作家")
            .description("指定用于将结果写入流文件的控制器服务。记录写入器可以使用继承模式来模拟推断模式行为，即，不需要在写入器中定义显式模式，并且将由用于从列类型推断模式的相同逻辑提供。")
...
//                    + "an explicit schema need not be defined in the writer, and will be supplied by the same logic used to infer the schema from the column types.")
    ...
    public static final PropertyDescriptor NORMALIZE_NAMES = new PropertyDescriptor.Builder()
            ...
            .description("是否更改列名中的字符。例如，冒号和句点将被更改为下划线。")
