...
@CapabilityDescription("从目录中的文件创建流文件。NiFi将忽略它至少没有读权限的文件。")
@WritesAttributes({
...
//            .name("Keep Source File")
            .name("保留源文件")
            .description("如果正确，则文件在复制到内容存储库后不会被删除;这将导致文件被不断地拾取，这对于测试是很有用的。如果不保留原始的NiFi，则需要对它正在提取的目录具有写权限，否则它将忽略该文件。")
...
//            .name("File Filter")
            .name("文件过滤")
            .description("只有名字与给定正则表达式匹配的文件才会被选中")
...
//            .name("Path Filter")
            .name("路径过滤")
            .description("当 " + RECURSE.getName() + " 正确, 只扫描路径与给定正则表达式匹配的子目录")
            .required(false)
            .addValidator(StandardValidators.REGULAR_EXPRESSION_VALIDATOR)
            .build();
    public static final PropertyDescriptor MIN_AGE = new PropertyDescriptor.Builder()
            .name("最小文件年龄")
            .description("要提取文件必须的最小年龄;任何比这个时间短的文件(根据最后修改日期)都将被忽略")
...
//            .description("The minimum age that a file must be in order to be pulled; any file younger than this amount of time (according to last modification date) will be ignored")
    ...
    public static final PropertyDescriptor MAX_AGE = new PropertyDescriptor.Builder()
            .name("最大文件年龄")
            .description("要提取一个文件必须的最大时间;任何超过这个时间的文件(根据最后的修改日期)都将被忽略")
...
//            .description("Indicates whether or not hidden files should be ignored")
    ...
    public static final PropertyDescriptor POLLING_INTERVAL = new PropertyDescriptor.Builder()
            .name("轮询间隔")
            .description("指示执行目录列表之前需要等待多长时间")
...
//            .description("The maximum number of files to pull in each iteration")
    ...
    public static final Relationship REL_SUCCESS = new Relationship.Builder().name("成功").description("所有文件都被成功路由到").build();
