...
@CapabilityDescription("尝试按照RFC5424格式解析格式良好的Syslog消息的内容，并为Syslog消息的每个部分(包括结构化数据)添加属性到流文件。结构化数据将作为每个项目id +参数的一个属性写入属性，参见https://tools.ietf.org/html/rfc5424.Note: ParseSyslog5424比ParseSyslog更严格地遵循规范。如果您的Syslog日志生产者没有严格遵循规范，例如，对于缺少的头项使用“-”，这些日志将在这个解析器中失败，而在ParseSyslog中它们不会失败。")
