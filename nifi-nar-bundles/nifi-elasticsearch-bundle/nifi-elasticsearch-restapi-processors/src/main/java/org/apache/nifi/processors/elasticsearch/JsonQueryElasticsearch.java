...
@CapabilityDescription("一个处理器，允许用户运行用Elasticsearch JSON DSL编写的查询(带有聚合)。它不会为用户自动对查询进行分页。如果将传入关系添加到这个处理器，它将使用flowfile的内容进行查询。应该注意查询的大小，因为来自Elasticsearch的整个响应都将一次性加载到内存中，并转换为生成的流文件。")
