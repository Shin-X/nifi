...
@CapabilityDescription("这个处理器是一个记录感知处理器，用于向MongoDB插入数据。它使用配置的记录读取器和模式从流文件的主体中读取传入的记录集，然后将这些记录批量插入到配置的MongoDB集合中。此处理器不支持更新、删除或upserts。每次要插入的文档数量由\" insert Batch Size\"配置属性控制。这个值应该设置为一个合理的大小，以确保MongoDB不会因为一次插入太多而重载。")
