...
@CapabilityDescription("基于散列和范围键从DynamoDB中放置一个文档。该表既可以有散列和范围，也可以只有散列键。目前支持的键为字符串和数字，值可以为json文档。对于散列键和范围键，操作都需要这两个键。说明FlowFile的内容必须为JSON。FlowFile内容映射到DynamoDB项中指定的Json文档属性。")
@WritesAttributes({
