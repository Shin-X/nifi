...
@CapabilityDescription("基于对每个流文件的表达式语言表达式求值跟踪滚动窗口，并将该值添加到处理器的状态。每个流文件将被发出，并带有流文件的计数和当前时间窗口中处理的值的总聚合值。")
