...
@CapabilityDescription("从HDFS检索文件和目录的列表。这个处理器创建一个FlowFile(s)来表示包含相关信息的HDFS文件/dir。该处理器的主要目的是提供类似于HDFS Client的功能，如count, du, ls, test等。与ListHDFS不同，这个处理器是无状态的，支持传入的连接，并在dir级别上提供信息。"
        )
@WritesAttributes({
