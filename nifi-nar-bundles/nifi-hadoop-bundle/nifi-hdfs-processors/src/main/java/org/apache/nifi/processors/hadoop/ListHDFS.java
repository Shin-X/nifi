...
@CapabilityDescription("从HDFS检索文件列表。每次执行一个清单时，带有最新时间戳的文件将被排除，并在处理器的下一次执行中被取走。这样做是为了确保我们不会遗漏任何文件，或者在具有相同时间戳的文件紧接在处理器的一次执行前后写入的情况下，不会产生重复的文件。对于HDFS中列出的每个文件，该处理器创建一个流文件，它表示要与FetchHDFS一起获取的HDFS文件。这个处理器被设计成只在集群中的主节点上运行。如果主节点发生变化，新的主节点将从上一个节点停止的地方开始，而不会复制所有数据。与GetHDFS不同，该处理器不删除HDFS中的任何数据。")
@WritesAttributes({
