...
@CapabilityDescription("这个处理器轮询由HdfsAdmin API提供的通知事件。因为这使用了HdfsAdmin api，所以需要以HDFS超级用户的身份运行。目前有六种类型的事件(追加、关闭、创建、元数据、重命名和取消链接)。" +
        "请参阅org.apache.hadoop.hdfs.inotify。每个事件的完整解释的事件文档。这个处理器将根据定义的持续时间轮询新的事件。" +
        "对于接收到的每个事件，将创建一个新的流文件，其中包含预期的属性，并将事件本身序列化为JSON，并写入流文件的内容。" +
        "例如，if event。类型为APPEND，那么流文件的内容将包含包含关于APPEND事件的信息的JSON文件。如果成功，则将流程文件发送到“success”关系。" +
        "注意生成的流文件的存储位置。如果流文件存储在处理器的一个监视目录中，那么将会有一个永不结束的事件流。同样重要的是，要知道这个处理器必须使用所有事件。" +
        "筛选必须在处理器中进行。这是因为HDFS admin的事件通知API没有过滤。")
