...
@CapabilityDescription("从一个流文件的内容创建一个AMQP消息，并将消息发送给一个AMQP交换。在一个典型的AMQP交换模型中，发送到AMQP交换的消息将基于“Routing Key”被路由到它在队列中的最终目的地(绑定)。如果由于一些错误配置而没有建立Exchange、Routing Key和Queue之间的绑定，则消息将没有最终目的地并将返回(即数据不会进入队列)。如果发生了这种情况，你会在app-log和公告中看到这样的日志，流文件将被路由到“失败”关系。")
