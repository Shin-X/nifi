...
@CapabilityDescription("将流文件的内容发送到Windows Azure事件集线器。注意:在发送之前，流文件的内容将被缓冲到内存中，因此应该小心避免向这个处理器发送超过可用Java堆空间的流文件。另外，请注意，此处理器为事件集线器客户机创建了一个4线程的线程池。它们将是额外的线程，而不是为这个处理器调度的并发任务。")
