...
@CapabilityDescription("消费类型为BytesMessage、TextMessage、ObjectMessage、MapMessage或StreamMessage的JMS消息，将其内容转换为一个流文件，并将其转换为“成功”关系。JMS属性(如头和属性)将作为流文件属性复制。" +
        "MapMessages将被转换成json，然后再转换成字节数组。其他类型将把它们的原始内容作为字节数组传输到流文件中。")
