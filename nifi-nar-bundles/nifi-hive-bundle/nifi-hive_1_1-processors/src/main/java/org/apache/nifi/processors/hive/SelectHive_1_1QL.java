...
@CapabilityDescription("对Hive数据库连接执行提供的HiveQL SELECT查询。查询结果将转换为Avro或CSV格式。使用流是为了支持任意大的结果集。可以使用标准调度方法将该处理器调度为在计时器或cron表达式上运行，也可以由传入的流文件触发。如果它是由传入的流文件触发的，那么该流文件的属性将在评估select查询时可用。“selecthiveql.row FlowFile属性。指示被选中的行数。")
@WritesAttributes({
