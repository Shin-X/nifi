...
public class RollbackOnFailure {
    ...
    public static  PropertyDescriptor createRollbackOnFailureProperty(String additionalDescription) {
        return new PropertyDescriptor.Builder()
                ...
                .displayName("回滚失败")
                .description("指定如何处理错误。默认情况下(false)，如果在处理一个流文件时发生错误，该流文件将根据错误类型路由到'failure'或'retry'关系，处理器可以继续处理下一个流文件。相反，您可能希望回滚当前处理的流文件，并立即停止进一步的处理。在这种情况下，可以通过启用“失败时回滚”属性来实现这一点。如果启用，失败的流程文件将保留在输入关系中，而不会惩罚它，并被反复处理，直到它被成功处理或通过其他方法删除。重要的是要设置足够的“收益率持续时间”，以避免过于频繁地重试。" + additionalDescription)
