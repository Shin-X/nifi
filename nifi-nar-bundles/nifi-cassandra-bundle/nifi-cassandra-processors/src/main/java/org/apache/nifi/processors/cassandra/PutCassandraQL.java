...
@CapabilityDescription("在Cassandra 1上执行Cassandra Query Language (CQL)语句。x, 2。x,或3.0。x集群。传入流文件的内容应该是要执行的CQL命令。CQL命令可能会使用?为了逃避参数。在这种情况下，要使用的参数必须作为命名约定为cql.args.N的FlowFile属性存在。类型和cql.args.N。取值，其中N为正整数。cql.args.N。type应该是表示Cassandra类型的小写字符串。")
@ReadsAttributes({
