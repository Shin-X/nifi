...
@CapabilityDescription("消费来自Apache Kafka的消息，这些消息是专门针对Kafka 2.0消费者API构建的。" +
        "用于发送消息的补充性NiFi处理器是publishkafkarrecord_2_0。请注意，此时处理器假定从给定分区检索的所有记录都具有相同的模式。" +
        "如果任何Kafka消息被拉出，但是不能被解析或写入配置的记录读取器或记录写入器，消息的内容将被写入一个单独的流文件，" +
        "该流文件将被传输到'parse.failure'的关系。否则，每个流程文件都被发送到“成功”关系，并且可能在单个流程文件中包含许多单独的消息。" +
        "一个的记录。添加了一个‘record.count’属性来指示流文件中包含多少消息。没有两个Kafka消息将被放置在相同的流文件，如果他们有不同的模式，" +
        "或者如果他们有不同的消息头值，包含在<Headers to Add as Attributes> 属性中。")
